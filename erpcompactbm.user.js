...
function letsJQuery() {
  ...
  content.append('<div id="myStatBoxR" style="z-index:100; position: absolute; top: 300px; left: 590px; width: 160px; opacity: 0.8;"></div>');
    ...
    $j('div#MHPSet').before('<div id="MHP" style="position:relative;top:-380px;width:236px;left:250px;clear:both;padding:3px;margin:5px 0;font-weight:bold;color:#000;text-align:center;z-index:999">' + 'Energy:&nbsp;' + '<input id="multihit_count" style="text-align:right;" type="text" size="3" maxlength="4" value="1" />' + '&nbsp;X 10&nbsp;' + '<button id="multihit_start">HIT!</button><br />' );
  ...
  $j("body").ajaxSuccess(function (e, res, opt) {
    ...
    if (opt.url.indexOf('/battle-stats/') > -1 && unsafeWindow.SERVER_DATA.onlySpectator != 1) {
      ...
      for (var i = 0; i < history.length; i++) {
        ...
        top5HIST += '<tr><td><a target="_blank" href="http://www.erepublik.com/en/citizen/profile/' + bh.fightersData[history[i].citizen_id].id + '">' + bh.fightersData[history[i].citizen_id].name + '</a></td><td>' + history[i].kills + '</td><td><strong>' + numberWithCommas(history[i].damage) + '</strong></td></tr>';
